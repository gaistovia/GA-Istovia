...
**Status: the minimal version of this is live.** Both
`start-a-project.html` and the hidden Client Brief page post through the
same client-side function, `submitToGoogleSheets(payload)` in
`assets/js/modules/integrations.js`, which is connected to a deployed Apps
Script Web App today. What's deployed right now only appends the row and
returns the Project ID — it does **not** yet call
`createClientDriveFolder()` / `sendCompanyNotification()` /
`sendClientConfirmation()` shown in the fuller example below. Those are
the natural next additions to the same `doPost()` function.

1. In your Google Sheet: **Extensions → Apps Script**.
2. Write a `doPost(e)` entry point. This is the fuller version — the
   currently-deployed one is a subset (just the sheet append + ID):
...
function doPost(e) {
  ...
  createClientDriveFolder(projectId, payload);    // §3 — not yet added to the live script
  sendCompanyNotification(payload, projectId);     // §7 — not yet added to the live script
  sendClientConfirmation(payload, projectId);      // §7 — not yet added to the live script
...
4. In `assets/js/modules/integrations.js`, `submitToGoogleSheets()` is
   already wired to call that URL:
...
export async function submitToGoogleSheets(payload) {
  try {
    const res = await fetch(SHEETS_WEBHOOK_URL, {
      method: "POST",
      // Deliberately text/plain, not application/json — see note below.
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify(payload)
    });
    const data = await res.json();
    return { ok: Boolean(data && data.ok), projectId: (data && data.projectId) || null };
  } catch (err) {
    console.error("submitToGoogleSheets() request failed:", err);
    return { ok: false, projectId: null }; // fail soft — wizard still completes with a temp reference
  }
}
```

**Why `text/plain` and not `application/json`:** Apps Script Web Apps
don't respond to CORS preflight (`OPTIONS`) requests. A real
`Content-Type: application/json` header makes the browser send a
preflight first, which Apps Script never answers, so the browser blocks
the whole request. Sending `text/plain` keeps the request a CORS "simple
request" (no preflight), while the *body* is still valid JSON text —
`JSON.parse(e.postData.contents)` on the Apps Script side works exactly
the same regardless of what the header says. This is a real, easy-to-miss
gotcha; keep the header as `text/plain` in any future version of this
function.
...
| File | Status |
|---|---|
| `assets/js/modules/integrations.js` | `submitToGoogleSheets()` is **live**. Everything else can stay as-is or be called from the future Admin Dashboard |
| Apps Script (lives in Google, not this repo) | `doPost()` + `getNextProjectId()` are **live and deployed**. `createClientDriveFolder()`, `generateQuotation()`, `generateContract()`, `sendTemplatedEmail()`, optional `doGet()` for the portal are the natural next additions to the same script |
| `client-portal-*.html` | Still static placeholder content. Add one small script block to fetch + populate from `doGet()` (§8) — no layout changes |
