...
It is **Phase 1–2** of a longer-term Client Management Platform (GCMS).
**As of this revision, one piece of the backend is genuinely live:**
`submitToGoogleSheets()` is connected to a real deployed Google Apps
Script Web App, which appends every submission to a real Google Sheet and
returns a real sequential Project ID. Everything else described in this
guide (Drive automation, quotation/contract PDFs, the 9 email templates,
the Admin Dashboard) is still a documented placeholder — the integration
points are ready, but nothing calls them yet. See
...
└── assets/
    ...
    └── js/
        ...
        └── modules/
            ...
            └── integrations.js         Sheets intake is LIVE; Drive/PDF/email calls still placeholders
```
