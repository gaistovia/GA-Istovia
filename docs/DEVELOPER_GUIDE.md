...
`docs/phase3-integration-guide.md` for the exact steps to wire that in.
...
├── README.md
├── docs/
│   ├── phase3-integration-guide.md     Step-by-step: wiring real Google Apps Script
...
| Client Portal | `client-portal-<slug>.html` — read-only status page (Project ID, status timeline, notes, contacts, and placeholder buttons for quotation/contract/files/invoice). No login by design; access is the secret URL alone. |
| Admin Dashboard | Does not exist yet, intentionally. `integrations.js` is written so a future dashboard's "approve lead" action can call `generateQuotation()`, `generateContract()`, and `createDriveFolder()` directly. |
